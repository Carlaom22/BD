...
def register_user(user_type):
    ...
    if not request.json:
        return jsonify({"status": 400, "errors": "Bad request, JSON data is required"}), 400
    ...
    try:
        if user_type == 'doctor':
            if not all([medical_license, specialization_name, specialization_id]):
                return jsonify({"status": 400, "errors": "Missing fields for doctor registration"}), 400

            # Insert doctor with all the required details
            user_id = insert_doctor(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details, medical_license, specialization_id, specialization_name)
        elif user_type == 'patient':
            # Insert patient with all the required details
            medical_history = data.get('medical_history')
            user_id = insert_patient(cursor, username, email, password, name, date_of_birth, address, phone_number, medical_history)
        elif user_type == 'nurse':
            # Insert nurse with all the required details
            user_id = insert_nurse(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details, hierarchical_category)
        elif user_type == 'assistant':
            # Insert assistant with all the required details
            user_id = insert_assistant(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details)
...
def insert_user(cursor, user_type, username, email, password, name, date_of_birth, address, phone_number, contract_details):
    ...
    """
    cursor.execute(employee_sql, (username, email, password, name, date_of_birth, address, phone_number, contract_details))
...
def insert_doctor(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details, medical_license, specialization_id, specialization_name):
    # First, insert the doctor as an employee to get the employee_id
    employee_id = insert_user(cursor, 'doctor', username, email, password, name, date_of_birth, address, phone_number, contract_details)
    ...
    VALUES (%s, %s, %s, %s);
    """
    cursor.execute(doctor_sql, (medical_license, specialization_id, specialization_name, employee_id))
    
    return employee_id


def insert_nurse(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details, hierarchical_category):
    # Insert employee and get the employee_id
    employee_id = insert_user(cursor, 'nurse', username, email, password, name, date_of_birth, address, phone_number, contract_details)
...
def insert_assistant(cursor, username, email, password, name, date_of_birth, address, phone_number, contract_details):
    # Insert employee and get the employee_id
    employee_id = insert_user(cursor, 'assistant', username, email, password, name, date_of_birth, address, phone_number, contract_details)
...
def insert_patient(cursor, username, email, password, name, date_of_birth, address, phone_number, medical_history):
    try:
        ...
        """, (username, email, password, name, date_of_birth, address, phone_number, medical_history))
...
def schedule_appointment():
    if not request.json or 'doctor_id' not in request.json or 'date' not in request.json:
    ...
    if token:
        ...
        try:
            conn = db_connection()
            cur = conn.cursor()
            cur.execute("SELECT patient_id FROM patient WHERE username = %s", (username,))
            patient_id = cur.fetchone()
            if patient_id is None:
                return jsonify({"status": 404, "errors": "No patient found with the given username"}), 404
    ...
    try:
        ...
        """, (doctor_id, patient_id, date, purpose))  # Include purpose in the insert statement
    ...
    try:
        ...
        if patient_id:  # If the username belongs to a patient
            ...
            if employee_id:  # If the username belongs to an employee
                ...
                cursor.execute("SELECT employee_employee_id FROM doctor_specialization WHERE employee_employee_id = %s;", (employee_id[0],))
                doctor_id = cursor.fetchone()

                if doctor_id:  # If the employee is a doctor
                    cursor.close()
                    conn.close()
                    return 'doctor', doctor_id[0]  # Return role as 'doctor' and doctor_id

                # Check if the employee is a nurse
                cursor.execute("SELECT employee_employee_id FROM nurse WHERE employee_employee_id = %s;", (employee_id[0],))
                ...
                cursor.execute("SELECT employee_employee_id FROM assistant WHERE employee_employee_id = %s;", (employee_id[0],))
                assistant_id = cursor.fetchone()

                if assistant_id:  # If the employee is an assistant
                    cursor.close()
                    conn.close()
                    return 'assistant', assistant_id[0]  # Return role as 'assistant' and assistant_id
...
def see_appointments(patient_user_id):
    ...
    if user_role not in ['assistant', 'patient'] or (user_role == 'patient' and user_id != patient_user_id):
        return jsonify({"status": 403, "errors": "You are not authorized to view these appointments"}), 403
    ...
    try:
        ...
        # Format the results
        results = [
            {"id": appt[0], "doctor_id": appt[1], "date": appt[2].strftime("%Y-%m-%d"), "doctor_name": appt[3]}
...
def schedule_surgery(hospitalization_id=None):
    ...
    try:
        ...
        cursor.execute("INSERT INTO bill_ (total_amount, status_) VALUES (%s, %s) RETURNING bill_id;", (1000, False))
        ...
        if not hospitalization_id:
            ...
            if not primary_nurse_id:
                return jsonify({"status": 400, "errors": "No primary nurse provided"}), 400
            
            # Create new hospitalization with the new bill_id and primary nurse id
            cursor.execute("INSERT INTO hospitalization (patient_patient_id, admission_date, nurse_employee_employee_id, bill__bill_id) VALUES (%s, %s, %s, %s) RETURNING hospitalization_id;", (patient_id, date_surgery, primary_nurse_id, bill_id))
            hospitalization_id = cursor.fetchone()[0]

        # Insert new surgery
        cursor.execute("INSERT INTO surgery (doctor_specialization_employee_employee_id, hospitalization_hospitalization_id, date_surgery) VALUES (%s, %s, %s) RETURNING surgery_id;", (doctor_specialization_id, hospitalization_id, date_surgery))
        surgery_id = cursor.fetchone()[0]

        # Associate nurses with the surgery
        for nurse in nurses:
            cursor.execute("INSERT INTO nurse_surgery (nurse_employee_employee_id, surgery_surgery_id) VALUES (%s, %s);", (nurse[0], surgery_id))
...
def get_prescriptions(person_id):
    ...
    token = request.headers.get('Authorization', '').split(' ')[1]
    user_role, user_id = get_user_role_and_id_from_token(token)
    if user_role not in ['assistant', 'patient'] or (user_role == 'patient' and user_id != person_id):
        return jsonify({"status": 403, "errors": "Unauthorized access"}), 403
    ...
    try:
        ...
        if not prescriptions:
            return jsonify({"status": 404, "errors": "No prescriptions found", "results": []}), 404
...
def add_prescription():
    ...
    user_role, user_id = get_user_role_and_id_from_token(token)
    if user_role != 'doctor':
        return jsonify({"status": 403, "errors": "Unauthorized access, only doctors can use this endpoint"}), 403

    # Ensure JSON data is provided
    data = request.json
    if not data or 'type' not in data or 'event_id' not in data or 'validity' not in data or 'medicines' not in data:
        return jsonify({"status": 400, "errors": "Missing required fields"}), 400
    ...
    try:
        ...
        for medicine in medicines:
            ...
            if not result:
                ...
                return jsonify({"status": 404, "errors": f"Medicine not found in the database: {medicine['medicine']}"}), 404
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s);
            """, (prescription_id, medicine['posology_dose'], medicine['posology_frequency'], medicine_id))
...
def execute_payment(bill_id):
    ...
    if user_role != 'patient':
        return jsonify({"status": 403, "errors": "Unauthorized access, only patients can pay their bills"}), 403
    ...
    try:
        ...
        if not bill:
            conn.rollback()
            return jsonify({"status": 404, "errors": "Bill not found or does not belong to the patient"}), 404
...
@app.route('/dbproj/daily/<string:year_month_day>', methods=['GET'])  # Use 'string' instead of 'date'
@token_required
def get_daily_summary(year_month_day):
    ...
    try:
        ...
        return jsonify({"status": 400, "errors": "Invalid date format, expected YYYY-MM-DD"}), 400
    ...
    """, (date_object, date_object, date_object, date_object, date_object, date_object))
        ...
        return jsonify({"status": 200, "results": {
            "amount_spent": result['amount_spent'] if result['amount_spent'] else 0,
...
def generate_monthly_report():
    ...
    try:
        ...
        report = [{"month": month, "doctor": entry["doctor"], "surgeries": entry["surgeries"]} for month, entry in report_dict.items()]

        # Return the JSON response
        return jsonify({"status": StatusCodes['success'], "results": report}), StatusCodes['success']
    except Exception as e:
        
        return jsonify({"status": StatusCodes['internal_error'], "errors": str(e)}), StatusCodes['internal_error']
    finally:
